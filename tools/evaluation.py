...
class evaluation_ntu_single(object):
    ...
    def bone_length_difference(self):
        if self.dataset == 'ntu':
            ...
            refboneVecs = self.target_sample[:, :, self.neighbor_link[:, 0]] - self.target_sample[:, :, self.neighbor_link[:, 1]]
            ...
            bone_diff = (np.abs(refboneLengths - adboneLengths)) / np.abs(refboneLengths)
            bone_diff[np.isnan(bone_diff)] = 0
            bone_diff[np.isinf(bone_diff)] = 0
            bone_diff = np.reshape(bone_diff,(-1,1))
            #itemindex = np.argwhere(bone_diff == 0)
            #bone_diff = np.delete(bone_diff,itemindex)
            bone_diff = np.mean(bone_diff)
            return bone_diff
