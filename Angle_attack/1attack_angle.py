...
def main():
    ...
    for target_sample_data_old, target_label_old in Target_data:
        ...
        target_sample_process = tools.postprocessing(target_sample.cpu().numpy(), core)
        target_sample_ik,target_sample_euler = kinematics.target_ik(target_sample_process)
        ...
        for initial_sample_data_old, initial_label_old in Initial_data:
            ...
            while True:
                ...
                trial_sample_ik = target_sample_ik.copy()
                trial_sample_ik = kinematics.animation_euler(trial_sample_euler,trial_sample_ik)
                trial_sample_postpone = Animation.positions_global(trial_sample_ik)
                trial_sample = torch.from_numpy(tools.normalize(trial_sample_postpone[:,1:,:], core)).float().cuda()
            ...
            while True:
                ...
                while True:
                    ...
                    for i in torch.arange(10):
                        ...
                        trial_sample_ik = kinematics.animation_euler(trial_sample_euler,trial_sample_ik)
                        or_p_root = tools.orthogonal_perturbation(delta,adversarial_sample, target_sample).cpu().numpy()
                        adversarial_sample_root_process = tools.postprocessing(adversarial_sample.cpu().numpy(),core)[:,0,:]
                        or_p_root_process = tools.postprocessing(or_p_root,core)[:,0,:]
                        trial_sample_root = adversarial_sample_root_process + or_p_root_process
                        trial_sample_ik.positions[:,0,:] = trial_sample_root
                        trial_sample_postpone = Animation.positions_global(trial_sample_ik)
                        trial_sample = torch.from_numpy(tools.normalize(trial_sample_postpone[:,1:,], core)).float().cuda()
                        trial_sample_data[0, :, :, :, 0] = trial_sample
                        trial_output = model(trial_sample_data)
                    ...
                    for i in torch.arange(prediction_len):
                        if predictions_value[i] == target_class:
                            ...
                            if d_score > 0.0:
                                ...
                                m_np = (predictions == predictions_value[i]).cpu().numpy()
                                adversarial_sample = trial_samples[torch.where(predictions == predictions_value[i])[0][0]]
                                adversarial_sample_euler = trial_samples_euler[np.where(m_np)[0][0]]
                ...
                for i in torch.arange(attack_len):
                    while True:
                        ...
                        trial_sample_ik = kinematics.animation_euler(trial_sample_euler,trial_sample_ik)
                        trial_sample_root = adversarial_samples_op[i] + forward_perturbation(
                            epsilon_op[i] * get_diff(adversarial_samples_op[i], target_sample),
                            adversarial_samples_op[i], target_sample)
                        trial_sample_root = tools.postprocessing(trial_sample_root.cpu().numpy(),core)[:,0,:]
                        trial_sample_ik.positions[:,0,:] = trial_sample_root
                        trial_sample_postpone = Animation.positions_global(trial_sample_ik)
                        trial_sample = torch.from_numpy(tools.normalize(trial_sample_postpone[:,1:,], core)).float().cuda()
                        #trial_sample = np.transpose(trial_sample, [0, 2, 1])
                        trial_sample = trial_sample.reshape(1, 3, 60, 25)
