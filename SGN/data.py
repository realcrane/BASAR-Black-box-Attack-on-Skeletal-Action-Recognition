...
class NTUDataLoaders(object):
    ...
    def create_datasets(self):
        ...
        if self.dataset == 'NTU':
            ...
            path_load = '/usr/not-backed-up/dyf/code/data/hdm05/sgn/'
            self.train_X = np.load(os.path.join(path_load, 'train_sgn_data.npy'))
            self.train_Y = np.load(os.path.join(path_load, 'train_label.npy'))
            self.val_X = np.load(os.path.join(path_load, 'val_sgn_data.npy'))
            self.val_Y = np.load(os.path.join(path_load, 'val_label.npy'))
            self.test_X = np.load(os.path.join(path_load, 'val_sgn_data.npy'))
            self.test_Y = np.load(os.path.join(path_load, 'val_label.npy'))
        elif self.dataset == 'kinetics':
            path_load = '/usr/not-backed-up/dyf/code/data/kinetics/sgn/'
            #path_load = '/usr/not-backed-up/dyf/code/MS-G3D/data/kinetics/'
            self.train_X = np.load(os.path.join(path_load, 'train_sgn_data.npy'))
            self.train_Y = np.load(os.path.join(path_load, 'train_label.npy'))
            self.val_X = np.load(os.path.join(path_load, 'val_sgn_data.npy'))
            self.val_Y = np.load(os.path.join(path_load, 'val_label.npy'))
