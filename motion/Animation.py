...
def transforms_multiply(t0s, t1s):
    ...
    """
    if t0s.dtype == 'object':
        z1 = np.array([np.dot(t0s[k], t1s[k]) for k in range(len(t0s))])
    else:
        z1 = ut.matrix_multiply(t0s, t1s)
    return z1
...
def transforms_global(anim):
    ...
    if locals.dtype == 'object':
        globals = globals.astype(np.object)
    globals[:,0] = locals[:,0]
